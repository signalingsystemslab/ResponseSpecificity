...
# Figure S1B gene ontology (unused)----
...
# Figure 1F extra heatmap (unused)----
macro = readRDS("./output/macrophage_M0_rep2only_500genes_DBEC.rds")
...
# Figure 2C gene expression heatmap in same order---------------------
macro = readRDS("./output/macrophage_M0_rep2only_500genes_DBEC.rds")
data = GetAssayData(object = macro, assay = "ISnorm", slot = "data")
data <- as.data.frame( as.matrix(data))
meta = macro@meta.data
...
# Figure 2D-E (and Fig5B) get dimension best----
...
# Figure 2F ML model on dimension best----
...
# Figure S2 (extra) test random sets of 15 genes----
store = data.frame()
set.seed(1)
for (i in c("3hr")){
...
# Figure S2 (extra) collect FPR and FDR for each stimulus from rnd15 ----
table = data.frame()
...
# Figure 2G channel capacity for each pairwise stimuli----
collect = data.frame()
...
############################################################################### Figure 3----
# Figure 3A-B mean vs variance contribution to channel capacity------
if(1){
...
############################################################################### Figure 4----
# Figure 4B-D gene expression pairwise MI----
...
# MI for signaling input data (unused)--------------------------
...
# Figure 4f signaling codon vs gene expression----
#plot features----
...
#################################
# REDO Figure 4F - signaling codons to expression ResponseSpecificity----
expt.info = readxl::read_excel("F://enhancer_dynamics/nfkb_trajectories_20190302/expt_info.xlsx")
features = read.delim("F://enhancer_dynamics/features.txt", sep = ",")
ggplot(features[grepl("610|547|756|566|754|783", features$ID), ], aes(as.factor(ID),off_times))+geom_boxplot(outlier.shape =NA)+
  geom_point(position = "jitter", aes(color = as.factor(ID)))+theme_bw(base_size = 18)
...
# Figure S5 (distance metric M0, M1, M2) (unused) ---- 
macro = readRDS("./output/macrophage_M0M1M2_combined_500genes_DBEC_3hr.rds")
...
# Figure 5F scatterplot M0 vs M1/M2----
...
# Figure 5G test M1/M2 on ML models----
...
################################################################################ Figure 6----
# Figure 6B pca of steady states vs responses and bhattacharya distances----
if (0){
  macro = readRDS("./output/macrophage_M0M1M2_combined_500genes_DBEC_3hr.rds")
...
# Figure 6C-D REDO all pairs by MI for RSS----
samptag.all = read.delim("./output/samptag.all_cellannotations_metadata.txt")
samptag.all$Cell_Index = paste0("X", samptag.all$Cell_Index)
...
# Figure 6E calculate MI on PC scores----
samptag.all = read.delim("./output/samptag.all_cellannotations_metadata.txt")
samptag.all$Cell_Index = paste0("X", samptag.all$Cell_Index)
...
# Figure S6 old (unused) plot and label loadings----
...
# Figure 7D ----
# OLDplot disease channel capacity differences--------
...
#####################################################
# Fig 7C disease model, REDO calc RSI pairwise----
samptag.all = read.delim("./output/samptag.all_cellannotations_metadata.txt")
samptag.all$Cell_Index = paste0("X", samptag.all$Cell_Index)
...
# Fig S7 revision motif enrichment----
library(enrichTF);library(RcisTarget)
# library(org.Mm.eg.db)
...
#################
# Fig S3 compare Rhapsody BMDM WT to iMPDM----
...
################
# Fig S3 violins rep1 vs rep2 iMPDMs----
...
################
# Fig S5A motif analysis on the categories of Fig4-----
...
pheatmap(frame.truncate[,c(2,4,6,5,3)]*-1, scale = "none", clustering_method = "ward.D2", 
         colorRampPalette((brewer.pal(n = 9, name = "Reds")))(103),
         breaks=c(0,seq(0.01,10,length=100),30),
         cluster_cols = F, cluster_rows = T, show_rownames = T, display_numbers = T ,number_color = "dodgerblue")
